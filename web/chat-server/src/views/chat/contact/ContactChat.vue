              ...
              <Modal :isVisible="isGroupContactInfoModalVisible">
                ...
                <template v-slot:body>
                  ...
                  >
                    ...
                    <el-descriptions-item label="Announcement" :span="3">
                      <div style="height: 70px">
                      ...
                      >
                          ...
                          :rules="[
                            {
                              min: 3,
                              max: 10,
                              message: 'Group name must be 3 to 10 characters.',
                              trigger: 'blur',
                            },
                          ]"
                        >
                        ...
                        <el-form-item prop="add_mode" label="Join Mode">
                          <el-radio-group v-model="updateGroupInfo.add_mode">
                            <el-radio :value="0">Join directly</el-radio>
                            <el-radio :value="1">Owner approval</el-radio>
                          </el-radio-group>
                        </el-form-item>
                        <el-form-item prop="notice" label="Announcement">
                          <el-input
                      ...
                      <ul class="addGroup-list" style="list-style-type: none">
                        ...
                        >
                          ...
                          <el-dropdown placement="right" trigger="click">
                            <el-button class="action-btn"> Review </el-button>
                            <template #dropdown>
                              <el-dropdown-menu>
                                <el-dropdown-item
                  ...
                  <div class="video-modal-content">
                    ...
                    <div class="video-modal-footer">
                      <el-button
                        ...
                        >Start Call</el-button
                      >
                      <el-button
                        class="video-modal-footer-btn"
                        @click="startCall(false)"
                        >Answer Call</el-button
                      >
                      <el-button
                        class="video-modal-footer-btn"
                        @click="rejectCall"
                        >Decline Call</el-button
                      >
                      <el-button
                        class="video-modal-footer-btn"
                        @click="sendEndCall"
...
export default {
  ...
  setup() {
    ...
    onBeforeRouteUpdate(async (to, from, next) => {
      ...
      store.state.socket.onmessage = (jsonMessage) => {
        ...
        if (message.type != 3) {
          if (
            // Group message and current session is that group
            (message.receive_id[0] == "G" &&
              message.receive_id == data.contactInfo.contact_id) ||
            // Other user's message and current session is that user
          ...
          // Other incoming messages aren't shown in messageList; reload via getMessageList on page change.
        } else {
          var messageAVdata = JSON.parse(message.av_data); // Backend field name is av_data
          if (messageAVdata.messageId === "CURRENT_PEERS") {
            console.log(
              "CURRENT_PEERS online list, curContactList:",
            ...
            if (messageAVdata.type === "start_call") {
              ElNotification({
                title: "Notification",
                message: `Incoming call request from ${message.send_name}. Please check it now.`,
                type: "warning",
              });
      ...
      try {
        ...
        if (rsp.data.code == 200) {
          ...
          if (data.addGroupList == null) {
            ElMessage.warning("No new group join requests");
            return;
          } else {
    ...
    const preToDeleteSession = () => {
      try {
        ElMessageBox.confirm("Delete this conversation and its chat history?", "Warning", {
    ...
    const preToDeleteContact = () => {
      try {
        ElMessageBox.confirm("Delete this contact?", "Warning", {
          confirmButtonText: "Confirm",
          cancelButtonText: "Cancel",
          type: "warning",
        })
    ...
    const preToBlackContact = () => {
      try {
        ElMessageBox.confirm("Block this contact?", "Warning", {
          confirmButtonText: "Confirm",
          cancelButtonText: "Cancel",
          type: "warning",
        })
  ...
  console.log("Avatar before upload====>", avatar);
    ...
    const handleUpdateGroupInfo = async () => {
      try {
        if (
          ...
          ElMessage.error("Please modify at least one field.");
          return;
        }
  ...
  ElMessage.warning("Please end the call first.");
    ...
    const createRtcPeerConnection = () => {
      ...
      data.rtcPeerConn.oniceconnectionstatechange = () => {
        console.log(
    ...
    const startCall = async (isInitiator) => {
      ...
      if (data.localVideo) {
  ElMessage.warning("Already in a call. Please don't start another.");
        return;
      }
      if (isInitiator && !data.ableToStartCall) {
        ElMessage.warning(
          "The other party has already started a call. Please answer or decline before starting another."
        );
        return;
      }
      if (!isInitiator && !data.ableToReceiveOrRejectCall) {
  ElMessage.warning("The other party hasn't started a call or is already in a call.");
        return;
      }
  ...
  ElMessage.warning("No call in progress.");
        return;
      }
  ...
  ElMessage.warning("The other party rejected the call");
    ...
    const receiveEndCall = () => {
      ...
      data.ableToReceiveOrRejectCall = false;
      data.ableToStartCall = true;
  ElMessage.warning("The other party hung up");
    };
    ...
    const rejectCall = () => {
      if (!data.ableToReceiveOrRejectCall) {
        ElMessage.warning(
          "The other party hasn't started a call or is already in a call, so you can't reject it."
        );
        return;
      }
